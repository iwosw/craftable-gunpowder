...
- Единый конфиг выпадения, компостницы, рецептов, генерации и свойств руды.
- Пять достижений с русскими и английскими названиями.

## Настройки и достижения

С версии **1.1.0** настройки находятся в `config/craftablegunpowder.json`, который создаётся при первом запуске. Можно менять шансы и количество выпадения, расход перегноя, выход рецептов, генерацию и прочность руды, включать и отключать механики и достижения. Изменения требуют перезапуска игры/сервера. [Все параметры и примеры](docs/CONFIG.ru.md).

Описанные ниже числа — значения по умолчанию. Новая ветка достижений ведёт от перегноя к собственному пороху; за крафт пороха по рецепту мода выдаётся 50 опыта.
...
Пример имени: `craftable-gunpowder-1.21.1-neoforge-1.1.0.jar` — файл для Minecraft **1.21.1 / NeoForge**.
...
[Локальная проверка 1.1.0: все 24 сборки и серверные проверки прошли успешно.](docs/VALIDATION.md)
